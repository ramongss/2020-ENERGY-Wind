...
BaseDir       <- getwd()
ResultsDir    <- paste(BaseDir, "Results-Month3", sep="/")
...
dataset <- raw_data[months(raw_data$PCTimeStamp) %in% month.name[10],] %>% as.data.frame()
...
ceemd <- CEEMD(sig = dataset$Power, tt = dataset$PCTimeStamp,
               ...
               trials = 100, verbose = T, max.imf = 5)
# PlotIMFs(ceemd)
...
save.image("CEEMD-results-cv.RData")
...
for (i in 1:length(meta.list)) {
  for (j in 1:length(pp.list)) {
    stack[[k]] <- train(y~.,data = stack.database.train,
                        method = meta.list[i],
                        trControl = control,
                        preProcess = c("center","scale",pp.list[j]),
                        tuneLength = 5,
                        trace = FALSE)
...
## 2 steps
{
  ...
  for (m in 1:length(model.list)) {
    ...
    for (c in 1:length(Comp.train[[m]])) {
      ...
      obsTE[[c]] <- matrix(seq(1,dim(Comp.test[[m]][[c]])[1]+1,1),ncol = h, byrow = TRUE)
...
## 2-step-ahead 
{
  ...
  for (p in 1:length(preprocess.list)) {
    stack[[p]] <- train(y~.,data = stack2.database.train,
                        method = meta.list,
                        trControl = control,
                        preProcess = c("center","scale",preprocess.list[p]),
                        trace = FALSE)
...
## One step ahead predictions dataframes
{
  ...
  save(pred_test_1, file = 'pred-day1-10minutes.RData')  
}
...
## Two step ahead predictions dataframes
{
  ...
  save(pred_test_2, file = 'pred-day1-20minutes.RData')  
}
...
## Three steps ahead predictions dataframes
{
  ...
  save(pred_test_3, file = 'pred-day1-30minutes.RData')  
}
